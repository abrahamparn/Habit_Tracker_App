...
class RandomHabitAdapter(
    ...
    inner class PagerViewHolder internal constructor(itemView: View) :
        ...
        private var title = itemView.findViewById<TextView>(R.id.pager_tv_title)
        private val startTime = itemView.findViewById<TextView>(R.id.pager_tv_start_time)
        private val imageLabel = itemView.findViewById<ImageView>(R.id.item_priority_level_2)
        private val countTime = itemView.findViewById<TextView>(R.id.pager_tv_minutes)
        private val button = itemView.findViewById<Button>(R.id.btn_open_count_down)

        fun bind(pageType: PageType, pageData: Habit) {
            title.setText(pageData.title)
            startTime.setText(pageData.startTime)
            countTime.setText(pageData.minutesFocus.toString())
            imageLabel.setImageResource(when(pageType){
